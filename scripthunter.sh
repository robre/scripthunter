...
~/go/bin/./hakrawler -js -url $target -plain -depth 2 -scope strict -insecure > $TMPDIR/hakrawl1.txt
...
cat $TMPDIR/jsdirs.txt | sort -u | while read jsdir; do
    ...
    cat $TMPDIR/ffuf.json | jq -r ".results[].url" | grep "\.js" | ~/go/bin/./unfurl format "%s://%d%:%P%p" | grep -iE "\.js$" | sort -u >$TMPDIR/ffuf_tmp.txt
